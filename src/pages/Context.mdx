...
### Don't create objects directly in props of Provider
...
const App = () => {
  ...
  return (
    <Provider value={{ something: value }}>
      <SomeChildComponent />
    </Provider>
  );
};
...
in this case reference remains the same and consumers will not re-render if provider re-renders.
